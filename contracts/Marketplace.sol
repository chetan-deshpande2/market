    ...
    EIP712Upgradeable
{
    ...
    mapping(address => mapping(bytes32 => bool))
        public orderIsCancelledOrCompleted;
    //seller => orderHash => Amount of ERC1155 tokens left to sell
    mapping(address => mapping(bytes32 => uint256))
        public amountOf1155TokensLeftToSell;
     ...
     */
    function restrictTokenToThisMarketplace(address tokenAddress, bool forbid)
        external
        onlyOwner
    {
     ...
     */
    function withdrawETH(uint256 amount, address payable receiver)
        external
        onlyOwner
    {
    ...
    function completeOrder(
        ...
        if (isAuction) {
            if (_sellerOrder.listingType == ListingType.FLOOR_PRICE_BID) {
                require(
                    _sellerOrder.user == msg.sender,
                    "Offer should be accepted by the seller"
                );
            } else {
                require(
                    _sellerOrder.user == msg.sender ||
                        _buyerOrder.user == msg.sender,
                    "User address doesnt match"
                );
                if (msg.sender == _buyerOrder.user)
                    require(
                        block.timestamp > _sellerOrder.deadline,
            ...
            require(
                _buyerOrder.user == msg.sender,
                "Buyer address doesnt match"
            );
        }
        if (isAuction)
            require(
                _buyerOrder.paymentToken != address(0),
                "Only ERC20 for auction"
            );

        bool isERC721 = checkERCType(_buyerOrder.tokenAddress);
        bool isNotBundleOrder = _sellerOrder.listingType !=
            ListingType.BUNDLE_BUY_NOW &&
            _sellerOrder.listingType != ListingType.BUNDLE_AUCTION;
        ...
        if (
            isSimpleERC1155sale &&
            orderIsCancelledOrCompleted[_sellerOrder.user][sellerHash] ==
            false &&
            amountOf1155TokensLeftToSell[_sellerOrder.user][sellerHash] == 0
        ) {
            amountOf1155TokensLeftToSell[_sellerOrder.user][
                sellerHash
            ] = _sellerOrder.quantity;
        }
        ...
        if (isSimpleERC1155sale) {
            //if all ERC1155 tokens are sold
            if (
                amountOf1155TokensLeftToSell[_sellerOrder.user][sellerHash] == 0
            ) {
                orderIsCancelledOrCompleted[_sellerOrder.user][
                    sellerHash
                ] = true;
            }
        } else {
        ...
        if (msg.sender == _sellerOrder.user) {
            orderIsCancelledOrCompleted[_buyerOrder.user][
                buildHash(_buyerOrder)
            ] = true;
        }
        ...
        //Transfer bundle
        if (!isNotBundleOrder)
            transferBundle(_sellerOrder, _buyerOrder, isERC721);
        ...
        transferCoins(payment, isNotBundleOrder);
     ...
     */
    function cancelOrder(Order calldata _usersOrder, Sig calldata _usersSig)
        external
    {
        ...
        emit CancelledOrder(
            _usersOrder.user,
            _usersOrder.tokenAddress,
            _usersOrder.tokenId,
            _usersOrder.listingType
        );
    }
    ...
    function prePurchase(
        ...
        require(
            UnknownToken(tokenAddress).supportsInterface(
                type(IPreSale1155).interfaceId
            ) ||
                UnknownToken(tokenAddress).supportsInterface(
                    type(IPreSale721).interfaceId
                ),
        ...
        bool shouldLazyMint = supportsLazyMint &&
            ((isERC721 &&
                needsLazyMint721(tokenAddress, ownerAddress, tokenId)) ||
                (!isERC721 &&
                    needsLazyMint1155(
                        tokenAddress,
                        ownerAddress,
        ...
        if (isERC721) {
            ...
            ) = IPreSale721(tokenAddress).getTokenInfo(
                    msg.sender,
                    tokenId,
                    eventId
                );
            ...
            require(
                shouldLazyMint ||
                    IERC1155(tokenAddress).balanceOf(ownerAddress, tokenId) >=
                    quantity,
                "Not enough tokens"
            );
            ...
            (
                ...
                address paymentToken,
                bool availableForBuyer
            ) = IPreSale1155(tokenAddress).getTokenInfo(
                    msg.sender,
                    tokenId,
                    quantity,
                    eventId
            ...
            IPreSale1155(tokenAddress).countTokensBought(
                msg.sender,
                tokenId,
                quantity,
                eventId
    ...
    function transferTokens(
        ...
        if (isERC721) {
            bool shouldLazyMint = supportsLazyMint &&
                needsLazyMint721(tokenAddress, from, tokenId);
            ...
            bool shouldLazyMint = supportsLazyMint &&
                needsLazyMint1155(tokenAddress, from, tokenId, quantity);

            if (shouldLazyMint) {
                ILazyMint1155(tokenAddress).lazyMint(to, tokenId, quantity);
            } else {
    ...
    function transferBundle(
        ...
        bool supportsLazyMint = supportsLazyMint(
            _sellerOrder.tokenAddress,
            isERC721
        );
        ...
        for (uint i = 0; i < _sellerOrder.bundleTokens.length; i++) {
            require(
                _sellerOrder.bundleTokens[i] == _buyerOrder.bundleTokens[i],
                "Wrong tokenId"
            );
            require(
                _sellerOrder.bundleTokensQuantity[i] ==
                    _buyerOrder.bundleTokensQuantity[i],
                "Wrong quantity"
            );
            ...
            if (isERC721) {
                ...
                if (
                    ...
                    ILazyMint721(_sellerOrder.tokenAddress).lazyMint(
                        _buyerOrder.user,
                        bundleTokenId
                    );
                } else {
                ...
                if (
                    ...
                    ILazyMint1155(_sellerOrder.tokenAddress).lazyMint(
                        _buyerOrder.user,
                        bundleTokenId,
                        bundleTokenQuantity
                    );
     ...
     */
    function transferCoins(PaymentInfo memory payment, bool isNotBundleOrder)
        private
    {
        ...
        if (ERC20Payment) {
            require(
                IERC20(payment.paymentToken).balanceOf(payment.buyer) >=
                    payment.amount,
                "Not enough balance"
            );
            require(
                IERC20(payment.paymentToken).allowance(
                    payment.buyer,
                    address(this)
                ) >= payment.amount,
        ...
        if (
            UnknownToken(payment.tokenAddress).supportsInterface(
                type(IRoyaltyDistribution).interfaceId
            )
        ) {
            IRoyaltyDistribution tokenContract = IRoyaltyDistribution(
            ...
            if (
                ...
                if (
                    isNotBundleOrder &&
                    tokenContract
                        .getTokenRoyaltyDistribution(payment.tokenId)
                        .length >
                    0
                ) {
                    RoyaltyShare[] memory royaltyShares = tokenContract
                        .getTokenRoyaltyDistribution(payment.tokenId);
                    (
                        address royaltyReceiver,
                        uint256 royaltyAmount
                    ) = IRoyaltyDistribution(payment.tokenAddress).royaltyInfo(
                            payment.tokenId,
                            payment.amount
                        );
            ...
            uint256 marketplaceFeeAmount = (transactionAmount *
    ...
    function payRoyaltyIERC2981(
        ...
        if (totalAmount > 0) {
            ...
            uint256 marketplaceFeeAmount = (totalAmount * marketplaceFee) /
                10000;
            ...
            uint256 amountToSeller = totalAmount -
                marketplaceFeeAmount -
                royaltyAmount;
            ...
            if (ERC20Payment) {
                ...
                if (amountToSeller > 0) payable(to).transfer(amountToSeller);
            }
        ...
        if (totalAmount > 0) {
            ...
            if (royaltyAmount > 0) {
                ...
                for (uint i = 0; i < royaltyShares.length; i++) {
                    address royaltyShareReceiver = royaltyShares[i]
                        .collaborator;
                    uint256 royaltyShare = (royaltyAmount *
                        royaltyShares[i].share) / 10000;
                    if (
                        royaltyShare > 0 && royaltiesLeftToPay >= royaltyShare
                    ) {
                        if (ERC20Payment) {
                            IERC20(payment.paymentToken).transferFrom(
                                payment.buyer,
                                royaltyShareReceiver,
                                royaltyShare
                            );
                        } else {
                            payable(royaltyShareReceiver).transfer(
                                royaltyShare
                            );
                        }
    ...
    function transferMarketingFee(
        ...
        uint256 superAdminFeeShare = (marketplaceFeeAmount * superAdminShare) /
            10000;
        ...
        if (ERC20Payment) {
            ...
            if (adminFeeShare > 0) {
                IERC20(paymentToken).transferFrom(buyer, admin, adminFeeShare);
            }
        } else {
            admin.transfer(adminFeeShare);
        }
    }
    ...
    function checkOrdersValidity(
        ...
        if (isNotBundleOrder) {
            ...
            if (isERC721) {
                require(
                    _buyerOrder.quantity == 1 && _sellerOrder.quantity == 1,
                    "Non-1 quantity"
                );
                require(
                    (supportsLazyMint &&
                        ...
                        IERC721(_sellerOrder.tokenAddress).ownerOf(
                            _sellerOrder.tokenId
                        ) ==
                        _sellerOrder.user,
                    "Not an owner"
                ...
                require(
                    _buyerOrder.quantity > 0 && _sellerOrder.quantity > 0,
                    "0 quantity"
                );
                require(
                    (supportsLazyMint &&
                        ...
                        IERC1155(_sellerOrder.tokenAddress).balanceOf(
                            _sellerOrder.user,
                            _sellerOrder.tokenId
                        ) >=
                        _buyerOrder.quantity,
        ...
        require(
            IERC721(_sellerOrder.tokenAddress).isApprovedForAll(
                _sellerOrder.user,
                address(this)
            ),
            "Not approved"
        ...
        if (!isAuction) {
            require(
                _sellerOrder.deadline >= block.timestamp &&
                    _buyerOrder.deadline >= block.timestamp,
                "Overdue order"
            );
        } else {
    ...
    function checkOrdersCompatibility(
        ...
        require(
            _buyerOrder.tokenAddress == _sellerOrder.tokenAddress ||
                _buyerOrder.listingType == ListingType.FLOOR_PRICE_BID,
            "Different tokens"
        );
        require(
            _sellerOrder.tokenId == _buyerOrder.tokenId ||
                !isNotBundleOrder ||
                _buyerOrder.listingType == ListingType.FLOOR_PRICE_BID,
            "TokenIDs dont match"
        );
        if (!isERC721 && isNotBundleOrder) {
            require(
                amountOf1155TokensLeftToSell[_sellerOrder.user][sellerHash] >=
                    _buyerOrder.quantity,
                "Cant buy that many"
            );
        }
        require(
            _sellerOrder.listingType == _buyerOrder.listingType,
            "Listing type doesnt match"
        );
        require(
            _sellerOrder.paymentToken == _buyerOrder.paymentToken,
            "Payment token dont match"
        );
        require(
            (isNotBundleOrder &&
                ((_sellerOrder.value <= _buyerOrder.value && isERC721) ||
                    ((_sellerOrder.value * _buyerOrder.quantity) <=
                        _buyerOrder.value &&
                        !isERC721))) ||
                (!isNotBundleOrder &&
        ...
        require(
            _sellerOrder.bundleTokens.length ==
                _buyerOrder.bundleTokens.length &&
                _sellerOrder.bundleTokensQuantity.length ==
                _buyerOrder.bundleTokensQuantity.length,
    ...
    function checkSignature(
        ...
        require(
            !orderIsCancelledOrCompleted[userAddress][orderHash],
     ...
     */
    function checkERCType(address tokenAddress)
        private
        returns (bool isERC721)
    {
        bool isERC721 = UnknownToken(tokenAddress).supportsInterface(
    ...
    function needsLazyMint721(
        ...
        return
            !ILazyMint721(tokenAddress).exists(tokenId) &&
     ...
     */
    function supportsLazyMint(address tokenAddress, bool isERC721)
        private
        returns (bool)
    {
        return
            (isERC721 &&
                UnknownToken(tokenAddress).supportsInterface(
                    type(ILazyMint721).interfaceId
                )) ||
            (!isERC721 &&
                UnknownToken(tokenAddress).supportsInterface(
                    type(ILazyMint1155).interfaceId
                ));
    }
            ...
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        ...
                        keccak256(
                            abi.encodePacked(_order.bundleTokensQuantity)
                        ),
                        _order.salt
                    )
                )
     ...
     */
    function recoverAddress(bytes32 hash, Sig calldata _sig)
        public
        view
        returns (address)
    {
